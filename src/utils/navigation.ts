...
/**
 * Find the parent page that the current path belongs to (e.g. the landing page generated by [...menu].astro)
 * @param currentPath the current page's path
 * @returns the parent page's NavbarLink object if found, otherwise undefined
 */
export function getParentLink(currentPath: string): NavbarLink | undefined {
    // Iterate all links in the navbar
    for (const link of navbarConfig.links) {
        // Check whether it has children and is not a LinkPreset enum
        if (typeof link !== "number" && link.children && link.children.length > 0) {
            // Check whether the children contain the current path
 ...
 * Use a normal page navigation when Swup is unavailable
 */
...
export function navigateToPage(
    ...
    if (typeof window !== "undefined" && (window as any).swup) {
        try {
            // Use Swup for a no-reload navigation
            if (options?.replace) {
...
/**
 * Check whether it is the home page
 */
