...
export function applyThemeToDocument(theme: LIGHT_DARK_MODE, force = false) {
    ...
    // Get the complete information about the current theme state
    ...
    // If neither a theme switch nor a code-theme update is needed, and this isn't a forced update, return directly
    ...
    requestAnimationFrame(() => {
        ...
        // Quickly remove the protection class on the next frame, using a microtask to ensure the DOM update has completed
        if (needsThemeChange) {
            // Use requestAnimationFrame to remove the transition guard class on the next frame
            requestAnimationFrame(() => {
