...
export function getNavbarTransparentModeForWallpaperMode(mode: WALLPAPER_MODE): string {
    ...
    return "semi"; // use the default semi mode in all other cases
}
...
function updateNavbarTransparency(mode: WALLPAPER_MODE) {
    ...
    // Reinitialize the semi-transparent mode scroll detection (if needed)
...
function showBannerMode() {
    ...
    // Hide the fullscreen wallpaper (controlled via CSS classes)
    ...
    if (!isAlreadyVisible && typeof document !== 'undefined') {
        // If it is currently hiding, remove the hiding class first
...
export function applyWallpaperModeToDocument(mode: WALLPAPER_MODE, force = false) {
    ...
    // If the mode hasn't changed and this isn't a forced update, return directly
    ...
    if (currentMode === WALLPAPER_BANNER && mode !== WALLPAPER_BANNER) {
        ...
        // The navbar also updates its transparency immediately
        updateNavbarTransparency(mode);
        // Wait for the transition animation to finish before performing the actual mode switch
        setTimeout(() => {
            document.documentElement.classList.remove('banner-hiding');
            executeApply();
        }, 600);
    ...
    // For an initial load or forced update, we may need to run some logic immediately, or wait for the DOM to be ready
    const apply = () => {
        executeApply();
    };
    ...
    function executeApply() {
        ...
        // Add the transition guard class
        ...
        // Remove enable-banner; showBannerMode re-adds it (if switching to Banner mode)
        // If switching away from Banner, remove it inside executeApply
        if (mode !== WALLPAPER_BANNER) {
            body.classList.remove('enable-banner');
        } else {
            body.classList.add('enable-banner');
        }
        // Add the corresponding CSS class based on the mode
        switch (mode) {
            case WALLPAPER_BANNER:
                showBannerMode();
                break;
        ...
        // Remove the transition guard class after the transition animation finishes
        setTimeout(() => {
            document.documentElement.classList.remove('is-wallpaper-transitioning');
        }, 600);
    }
    // Use requestAnimationFrame to run on the next frame and avoid flicker
