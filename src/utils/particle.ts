...
class Particle {
    ...
    update() {
        ...
        // If the particle goes out of bounds or becomes fully transparent, reposition it
        if (
            this.x > window.innerWidth ||
            this.x < 0 ||
            this.y > window.innerHeight + BOUNDARY_OFFSET ||
            this.y < -BOUNDARY_OFFSET || // disappeared from the top
            this.a <= 0
        ) {
            // If the particle has no limit
            if (this.limitArray[this.idx] === -1) {
                this.resetPosition();
            }
            // Otherwise the particle has a limit
            else {
...
function getRandom(option: string, config: ParticleConfig): any {
    let ret: any;
    let random: number;
    // Get a random value based on the option
    switch (option) {
        case "x":
        ...
        case "a":
            ...
            break;
        case "fnx":
            random = config.speed.horizontal.min + Math.random() * (config.speed.horizontal.max - config.speed.horizontal.min); // keep a small random movement in the x direction
            ret = function (x: number, y: number) {
                return x + random;
            };
            break;
        case "fny":
            random = -(config.speed.vertical.min + Math.random() * (config.speed.vertical.max - config.speed.vertical.min)); // random upward movement in the y direction
            ret = function (x: number, y: number) {
                return y + random;
            };
            break;
