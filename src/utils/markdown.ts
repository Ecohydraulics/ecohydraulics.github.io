 ...
 * Uses event delegation so it still works after Swup no-reload transitions
 */

export function initMarkdownActions() {
    if (typeof document === "undefined") return;
    // Remove the old listener (if any) to prevent duplicate binding
    // Note: since this uses an anonymous function and usually runs only once on page load, in a Swup environment it runs only once as long as this script is loaded in the main layout.
