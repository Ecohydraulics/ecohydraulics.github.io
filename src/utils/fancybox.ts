...
export async function initFancybox() {
    ...
    // Album image selector (only binds images not inside <a> tags, to avoid conflicting with link bindings)
    ...
    if (!hasImages) return;
    // Check whether Fancybox has already been initialized
    if (!Fancybox) {
...
export function cleanupFancybox() {
    if (!Fancybox) return; // if it was never loaded, no cleanup needed
