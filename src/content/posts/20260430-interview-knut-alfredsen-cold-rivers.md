---
title: "New Ecohydraulics Interview: Knut Alfredsen - Cold Rivers"
published: 2026-04-30
author: Sebastian
description: Knut Alfredsen on YouTube
cover: "/assets/images/banner-conferences.jpg"
coverInContent: false
tags: [Community, Announcement]
category: News
draft: false
---

## Knut Alfredsen Interview: Cold Rivers

The [Ecohydraulics video channel](https://www.youtube.com/@ecohydraulics9581) has landed another interview, this time with Dr. Knut Alfredsen from the Norwegian University of Science and Technology (NTNU). If your scientific comfort zone includes hydrology, hydropower, cold-climate rivers, habitat modeling, and the occasional icy field problem that refuses to behave like a textbook example, this one is worth a coffee break. So, those teaching, modeling, restoring, licensing, or simply trying to understand what rivers are doing while pretending to be predictable, check out the interview. It may not solve every Pandora's box in ecohydraulics, but it certainly opens a few worthwhile drawers.

<div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;margin:1.5rem 0;border-radius:8px">
  <iframe style="position:absolute;top:0;left:0;width:100%;height:100%;border:0" src="https://www.youtube.com/embed/oOrOwOnqqjs" title="Ecohydraulics interview: Knut Alfredsen" loading="lazy" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
</div>
